...
DECLARE
  r     record;
  plan  text := '';
  pg19  boolean := current_setting('server_version_num')::int >= 190000;
  total numeric;
BEGIN
  FOR r IN EXECUTE 'EXPLAIN (costs off, verbose) SELECT sum(v) FROM pps_gp' LOOP
    plan := plan || r."QUERY PLAN" || E'\n';
  END LOOP;
  IF (plan LIKE '%bounded_numeric_sum%') <> pg19 THEN
    RAISE EXCEPTION 'plan for a non-superuser does not match the branch: %', plan;
  END IF;
...
DECLARE
  pg19 boolean := current_setting('server_version_num')::int >= 190000;
BEGIN
  IF pps_plan_has_rewrite() <> pg19 THEN
    RAISE EXCEPTION 'generic plan before detach does not match the branch';
  END IF;
END
$$;
...
DO $$
BEGIN
  IF pps_plan_has_rewrite() THEN
    RAISE EXCEPTION 'the generic plan still carries the rewrite after detach';
  END IF;
END
$$;
EXECUTE gp(0);
